...
def parse_inputs(input_lines):
    ...
    for line in input_lines.split('\n'):
        for idx, num in enumerate(line.strip().split()):
            columns[idx].append(int(num))
    all_triangles = [item for sublist in columns for item in sublist]
    return all_triangles


def valid_triangle(sites):
    results = []
    results.append(sites[0] + sites[1] > sites[2])
    results.append(sites[2] + sites[1] > sites[0])
    results.append(sites[2] + sites[0] > sites[1])
    if sum(results) == 3:
        return 1
    else:
        return 0
...
def solution_part2():
    data_input = """541  588  421
      ...
      436  766  254"""
    correct = 0
    all_triangles = parse_inputs(data_input)
    for start, end in zip(range(0, len(all_triangles) - 2, 3),
                          range(3, len(all_triangles) + 1, 3)):
        tri = all_triangles[start:end]
        correct += valid_triangle(tri)
    return correct
